...
tongue_NPLS = sNPLS::sNPLS(X, as.matrix(Ycnt), 1, threshold_j=0, threshold_k=0, scale.X = FALSE, scale.Y=FALSE, center.X=FALSE, center.Y=FALSE)
...
X[is.na(X)] = 0 # sNPLS cannot handle NAs - very stupid
saliva_NPLS = sNPLS::sNPLS(X, as.matrix(Ycnt), 1, threshold_j=0, threshold_k=0, scale.X = FALSE, scale.Y=FALSE, center.X=FALSE, center.Y=FALSE)

saveRDS(saliva_NPLS, "./Models/NPLS/GOHTRANS_saliva_NPLS.RDS")
```

```{r NPLS tongue}
Y = as.numeric(as.factor(processedCytokine$mode1$GenderID))
Ycnt = Y - mean(Y)

X = processedCytokine$data
X[is.na(X)] = 0 # sNPLS cannot handle NAs - very stupid
cytokine_NPLS = sNPLS::sNPLS(X, as.matrix(Ycnt), 1, threshold_j=0, threshold_k=0, scale.X = FALSE, scale.Y=FALSE, center.X=FALSE, center.Y=FALSE)

saveRDS(cytokine_NPLS, "./Models/NPLS/GOHTRANS_cytokine_NPLS.RDS")
```

```{r test metadata - tongue}
timepoints = c(0, 3, 6, 12)
Fac = list(tongue_NPLS$T, tongue_NPLS$Wj, tongue_NPLS$Wk)

# Tongue
normalSubjectLoadings = processedTongue$mode1 %>% as_tibble() %>% mutate(Component_1 = Fac[[1]][,1])
transformedSubjectLoadings = transformPARAFACloadings(Fac, 2, moreOutput=TRUE)$Ftilde %>% as_tibble() %>% mutate(subject = rep(processedTongue$mode1$subject, each=4), newTimepoint = rep(timepoints, 39))
