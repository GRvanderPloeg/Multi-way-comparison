...
result = ncrossreg(Z, Ycnt, pi=0.95, method="L-BFGS", nstart=5, numCores=parallel::detectCores(), cvFolds=2)
```

```{r fit model}
model = CMTFtoolbox::acmtfr_opt(Z, Ycnt, 2, pi=0.95, method="L-BFGS", nstart=10, numCores=10)
```
...
# Tongue
normalSubjectLoadings = processedTongue$mode1 %>% as_tibble() %>% mutate(Component_1 = model$Fac[[1]][,1], Component_2 = model$Fac[[1]][,2])
...
# Estradiol
temp = transformedSubjectLoadings %>% left_join(tongueSampleMeta) %>% select(V1, V2,newTimepoint, Estradiol_pmol_ml)
...
# Testosterone
temp = transformedSubjectLoadings %>% left_join(tongueSampleMeta) %>% select(V1, V2,newTimepoint, Testosterone_nmol_L)
...
# LH
temp = transformedSubjectLoadings %>% left_join(tongueSampleMeta) %>% select(V1, V2,newTimepoint, LH_U_L)
uncorrectedP[1,5] = cor.test(temp$V1, temp$LH_U_L)$p.value
uncorrectedP[2,5] = cor.test(temp$V2, temp$LH_U_L)$p.value

# SHBG
temp = transformedSubjectLoadings %>% left_join(tongueSampleMeta) %>% select(V1, V2,newTimepoint, SHBG_nmol_L)
uncorrectedP[1,6] = cor.test(temp$V1, temp$SHBG_nmol_L)$p.value
uncorrectedP[2,6] = cor.test(temp$V2, temp$SHBG_nmol_L)$p.value

# MUC5B
temp = transformedSubjectLoadings %>% left_join(tongueSampleMeta) %>% select(V1, V2,newTimepoint, MUC_5_B_ug_ml)
uncorrectedP[1,7] = cor.test(temp$V1, temp$MUC_5_B_ug_ml)$p.value
uncorrectedP[2,7] = cor.test(temp$V2, temp$MUC_5_B_ug_ml)$p.value

# Chitinase
temp = transformedSubjectLoadings %>% left_join(tongueSampleMeta) %>% select(V1, V2,newTimepoint, Chitinase_AU_ml)
...
# S-igA
temp = transformedSubjectLoadings %>% left_join(tongueSampleMeta) %>% select(V1, V2,newTimepoint, S_IgA_ug_ml)
uncorrectedP[1,9] = cor.test(temp$V1, temp$S_IgA_ug_ml)$p.value
uncorrectedP[2,9] = cor.test(temp$V2, temp$S_IgA_ug_ml)$p.value

# Lysozyme
temp = transformedSubjectLoadings %>% left_join(tongueSampleMeta) %>% select(V1, V2,newTimepoint, Lysozyme_U_ml)
...
# Amylase
temp = transformedSubjectLoadings %>% left_join(tongueSampleMeta) %>% select(V1, V2,newTimepoint, Amylase_U_ml)
uncorrectedP[1,11] = cor.test(temp$V1, temp$Amylase_U_ml)$p.value
uncorrectedP[2,11] = cor.test(temp$V2, temp$Amylase_U_ml)$p.value

# Total protein
temp = transformedSubjectLoadings %>% left_join(tongueSampleMeta) %>% select(V1, V2,newTimepoint, Total_protein_ug_ml)
...
Fac = list(saliva_NPLS$T, saliva_NPLS$Wj, saliva_NPLS$Wk)

# Saliva
normalSubjectLoadings = processedSaliva$mode1 %>% as_tibble() %>% mutate(Component_1 = Fac[[1]][,1])
transformedSubjectLoadings = transformPARAFACloadings(Fac, 2, moreOutput=TRUE)$Ftilde %>% as_tibble() %>% mutate(subject = rep(processedSaliva$mode1$subject, each=4), newTimepoint = rep(timepoints, 39))

uncorrectedP = matrix(NA, nrow=1, ncol=14)
...
# Estradiol
temp = transformedSubjectLoadings %>% left_join(salivaSampleMeta) %>% select(V1, newTimepoint, Estradiol_pmol_ml)
uncorrectedP[1,3] = cor.test(temp$V1, temp$Estradiol_pmol_ml)$p.value
# uncorrectedP[2,3] = cor.test(temp$V2, temp$Estradiol_pmol_ml)$p.value

# Testosterone
temp = transformedSubjectLoadings %>% left_join(salivaSampleMeta) %>% select(V1, newTimepoint, Testosterone_nmol_L)
uncorrectedP[1,4] = cor.test(temp$V1, temp$Testosterone_nmol_L)$p.value
# uncorrectedP[2,4] = cor.test(temp$V2, temp$Testosterone_nmol_L)$p.value

# LH
temp = transformedSubjectLoadings %>% left_join(salivaSampleMeta) %>% select(V1, newTimepoint, LH_U_L)
uncorrectedP[1,5] = cor.test(temp$V1, temp$LH_U_L)$p.value
# uncorrectedP[2,5] = cor.test(temp$V2, temp$LH_U_L)$p.value

# SHBG
temp = transformedSubjectLoadings %>% left_join(salivaSampleMeta) %>% select(V1, newTimepoint, SHBG_nmol_L)
uncorrectedP[1,6] = cor.test(temp$V1, temp$SHBG_nmol_L)$p.value
# uncorrectedP[2,6] = cor.test(temp$V2, temp$SHBG_nmol_L)$p.value

# MUC5B
temp = transformedSubjectLoadings %>% left_join(salivaSampleMeta) %>% select(V1, newTimepoint, MUC_5_B_ug_ml)
uncorrectedP[1,7] = cor.test(temp$V1, temp$MUC_5_B_ug_ml)$p.value
# uncorrectedP[2,7] = cor.test(temp$V2, temp$MUC_5_B_ug_ml)$p.value

# Chitinase
temp = transformedSubjectLoadings %>% left_join(salivaSampleMeta) %>% select(V1, newTimepoint, Chitinase_AU_ml)
uncorrectedP[1,8] = cor.test(temp$V1, temp$Chitinase_AU_ml)$p.value
# uncorrectedP[2,8] = cor.test(temp$V2, temp$Chitinase_AU_ml)$p.value

# S-igA
temp = transformedSubjectLoadings %>% left_join(salivaSampleMeta) %>% select(V1, newTimepoint, S_IgA_ug_ml)
uncorrectedP[1,9] = cor.test(temp$V1, temp$S_IgA_ug_ml)$p.value
# uncorrectedP[2,9] = cor.test(temp$V2, temp$S_IgA_ug_ml)$p.value

# Lysozyme
temp = transformedSubjectLoadings %>% left_join(salivaSampleMeta) %>% select(V1, newTimepoint, Lysozyme_U_ml)
uncorrectedP[1,10] = cor.test(temp$V1, temp$Lysozyme_U_ml)$p.value
# uncorrectedP[2,10] = cor.test(temp$V2, temp$Lysozyme_U_ml)$p.value

# Amylase
temp = transformedSubjectLoadings %>% left_join(salivaSampleMeta) %>% select(V1, newTimepoint, Amylase_U_ml)
uncorrectedP[1,11] = cor.test(temp$V1, temp$Amylase_U_ml)$p.value
# uncorrectedP[2,11] = cor.test(temp$V2, temp$Amylase_U_ml)$p.value

# Total protein
temp = transformedSubjectLoadings %>% left_join(salivaSampleMeta) %>% select(V1, newTimepoint, Total_protein_ug_ml)
uncorrectedP[1,12] = cor.test(temp$V1, temp$Total_protein_ug_ml)$p.value
# uncorrectedP[2,12] = cor.test(temp$V2, temp$Total_protein_ug_ml)$p.value
...
print(uncorrectedP)
correctedP = matrix(p.adjust(uncorrectedP, "BH"), nrow=1, ncol=14)
print(correctedP)
```

```{r test metadata - cytokine}
Fac = list(cytokine_NPLS$T, cytokine_NPLS$Wj, cytokine_NPLS$Wk)

# Cytokine
timepoints = c(0, 3, 12)

normalSubjectLoadings = processedCytokine$mode1 %>% as_tibble() %>% mutate(Component_1 = Fac[[1]][,1])
transformedSubjectLoadings = transformPARAFACloadings(Fac, 2, moreOutput=TRUE)$Ftilde %>% as_tibble() %>% mutate(subject = rep(processedCytokine$mode1$subject, each=3), newTimepoint = rep(timepoints, 27))
